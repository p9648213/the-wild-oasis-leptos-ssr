...
#[derive(Debug, Clone, Hash, Eq, PartialEq)]
pub struct SingleCabinKey(pub u32);
...
pub async fn get_cabin(id: u32) -> Result<Cabin, String> {
    let client = create_client();
    let resp = client.from("cabins").select("*").eq("id", id.to_string()).single().execute().await.map_err(|e| e.to_string())?;
    let response = resp.error_for_status().map_err(|e| e.to_string())?;
    let body = response.text().await.map_err(|e| e.to_string())?;
    let cabin: Cabin = serde_json::from_str(&body).map_err(|e| e.to_string())?;
    Ok(cabin)
}
...
pub fn single_cabin_query(id: u32) -> QueryScope<SingleCabinKey, Result<Cabin, String>> {
    create_query(move |_| async move {get_cabin(id).await}, QueryOptions::default())
}